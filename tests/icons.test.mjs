...
test('cache includes all declared icons and only removes this project old caches', async () => {
  ...
  vm.runInNewContext(worker, {
    ...
    caches: { keys: async () => ['screenshot-trimmer-v8', 'screenshot-trimmer-v9', 'screenshot-trimmer-v10', 'subflow-v9'], delete: async key => deleted.push(key) }
  });
  let finished;
  handlers.activate({ waitUntil(promise) { finished = promise; } });
  await finished;
  assert.deepEqual(deleted, ['screenshot-trimmer-v8', 'screenshot-trimmer-v9']);
